...
public class ExcelUtil<T> {
    ...
    public Map<String, DecimalFormat> decimalFormatMap = new HashMap<String, DecimalFormat>();
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        if (ColumnType.STRING == attr.cellType() || ColumnType.TEXT == attr.cellType()) {
            ...
            cell.setCellValue(StringUtils.isNull(cellValue) ? attr.defaultValue() : attr.prefix() + cellValue + attr.suffix());
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                if (StringUtils.isNotEmpty(dateFormat) && StringUtils.isNotNull(value)) {
                    ...
                    if(ColumnType.NUMERIC == attr.cellType()){
                    ...
                    if(-1 != attr.scale()){
                        ...
                        DecimalFormat df = decimalFormatMap.get(attr.decimalFormat());
                        if(df == null){
                            df = new DecimalFormat(attr.decimalFormat());
                            decimalFormatMap.put(attr.decimalFormat(), df);
                        }
                        df.setRoundingMode(RoundingMode.valueOf(attr.roundingMode()));
                        cell.setCellValue(new BigDecimal(df.format((BigDecimal) value)).doubleValue());
                    }
                } else if (!attr.handler().equals(ExcelHandlerAdapter.class)) {
